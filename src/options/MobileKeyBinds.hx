...
class MobileKeyBinds extends MusicBeatState // Code desenvolvido dois anos atrás, mas ainda dá pro gasto
{
	...
	public function new()
	{
		...
		// bg
		var background:FlxSprite = new FlxSprite().loadGraphic(Paths.image('backgrounds/${FlxG.random.int(1, 2)}',
		...
		{
			save();
			MusicBeatState.switchState(new MikuOptions());
		});
		savebutton.resize(250, 50);
		savebutton.setLabelFormat("VCR OSD Mono", 24, FlxColor.BLACK, "center");

		exportbutton = new FlxUIButton(FlxG.width - 150, 25, "Exportar", () ->
		{
		...
		importbutton = new FlxUIButton(exportbutton.x, 100, "Importar", () ->
		{
